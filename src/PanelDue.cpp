...
void ChangeTab(ButtonBase *newTab)
{
	...
	if (newTab == tabControl && status == PrinterStatus::printing )
	{
	...
	Buzzer::Beep(errorBeepFrequency, errorBeepLength, nvData.touchVolume);		
							...
							// 	MessageLog::AppendMessage("'Control' Unavailable!");
	...
	if (newTab != currentTab)
	{
		...
		if (status == PrinterStatus::printing)
		{
				...
				//newTab->Press(false, 0);
				//currentTab->Press(true, 0);
				currentTab = currentTab;
				
				
			//	Buzzer::Beep(touchBeepFrequency, 800, Buzzer::MaxVolume);		// long beep to acknowledge it

					//	PopupAreYouSure(ev, "Unavailable While Printing!");
